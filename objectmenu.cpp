...
ObjectMenu::ObjectMenu(vector<GLuint> images, Vector2d pos):
	buttonImages(images),
	pos(pos),
	selection(RECT),
	direction(VERTICAL),
	buttonOffset(5.0f, 5.0f),
	buttonSize(64.0f, 64.0f),
	buttonSpacing(5.0f),
	numButtons(3)
{}
...
void ObjectMenu::HandleEvent(Event& event)
{
...
Vector2d ObjectMenu::GetDimensions() const
{
	float width, height;
	...
	if (direction == HORIZONTAL) width = (buttonOffset.x * 2) + (buttonSize.x * (float)numButtons) + (buttonSpacing * (float)(numButtons - 1));
	else if (direction == VERTICAL) height = (buttonOffset.y * 2) + (buttonSize.y * (float)numButtons) + (buttonSpacing * (float)(numButtons - 1));
	
		return Vector2d(width, height);
}

bool ObjectMenu::MouseInWindow(float x, float y) const
{
	Vector2d size = GetDimensions();
...
int ObjectMenu::GetSelectedButton(float x, float y) const
{
